...
class TestEmailService(unittest.IsolatedAsyncioTestCase):
    ...
    def _setup_mock_settings(self, mock_settings):
        ...
        mock_settings.mail_password = "test_pass"
    ...
    async def test_send_email_verify_success(self, mock_fastmail_class, mock_settings):
        mock_settings.mail_username = "test_user"
        mock_settings.mail_password = "test_pass"
        mock_settings.mail_from = "test_from@example.com"
        mock_settings.mail_port = 587
        ...
        self.assertEqual(called_conf.MAIL_PASSWORD, mock_settings.mail_password)
        ...
        expected_link = f"{email_service.settings.base_url}/auth/verify_email/{test_token}"
        self.assertEqual(message_schema.template_body["host"], email_service.settings.base_url)
    ...
    @patch("src.services.email.FastMail")
    @patch("src.services.email.settings")
    async def test_send_rest_password_email_success(self, mock_settings, mock_environment_class, mock_fastmail_class):
        self._setup_mock_settings(mock_settings)
        ...
        mock_template_instance.render.return_value = "<html>Reset HTML</html>"
        ...
        mock_environment_class.assert_called_once_with(loader=email_service.FileSystemLoader("templates"))
        ...
        self.assertEqual(message_schema.recipientes, [test_email])
    ...
    @patch("src.services.email.FastMail")
    @patch("src.services.email.settings")
    async def test_send_reset_password_email_failure(self, mock_settings, mock_environment_class, mock_fastmail_class):
        self._setup_mock_settings(mock_settings)
