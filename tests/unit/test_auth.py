...
class TestAuthService(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.mock_db = AsyncMock()  # db is likely passed as an async session
        ...
        self.mock_user = User(
            ...
            is_verified=True,  # user already verified
        )
        ...
        self.mock_unverified_user = User(
            ...
            created_at=datetime.now(timezone.utc),
            updated_at=datetime.now(timezone.utc),
            is_verified=False,  # user not verified
        )

        self.mock_settings = MockSettings()
        # Patch auth_service.settings globally for all tests in this class
    ...
    @patch("src.auth.auth.repository_users", new_callable=AsyncMock)  # Ensure repository_users is an AsyncMock
    async def test_get_current_user_success(self, mock_repository_users):
        ...
        # Assign return_value directly, as mock_repository_users is now an AsyncMock instance
    ...
    async def test_get_current_user_invalid_token_jwt_error(self, mock_repository_users):
        ...
        mock_repository_users.get_user_by_email.assert_not_called()  # Use assert_not_called for async mocks without spec
    ...
    async def test_create_email_verification_token_and_save(self, mock_repo_users, mock_repo_auth):
        ...
        mock_repo_auth.create_verification_token = AsyncMock()  # This specific method is made an AsyncMock
    ...
    async def test_save_verification_token(self, mock_repository_auth):
        ...
        mock_repository_auth.create_verification_token.return_value = None  # This method does not return something, just creates
    ...
    @patch("src.auth.auth.repository_users", new_callable=AsyncMock)
    @patch("src.auth.auth.repository_auth", new_callable=AsyncMock)
    @patch("src.auth.auth.jwt.decode")  # Mock jwt.decode for token validation
    async def test_verify_email_token_success(self, mock_jwt_decode, mock_repository_auth, mock_repository_users):
        ...
        # Simulate the payload that jwt.decode would return from the input token
        mock_jwt_decode.return_value = {"sub": email_to_verify, "type": "email_verification"}

        # Mock the VerificationToken object that would be retrieved from the DB
        mock_token_db = MagicMock(  # MagicMock is fine here as it's just a data object
            id=1,
            user_id=self.mock_unverified_user.id,
            token="any_jwt_token_string",  # This is the token string that get_verification_token would receive
            token_type="email_verification",
            expires_at=datetime.now(timezone.utc) + timedelta(hours=1),  # Token is not expired
            user=self.mock_unverified_user  # Ensure the token has the user associated if needed by auth.py logic
        )
        ...
        for attr, value in self.mock_unverified_user.__dict__.items():
            if not attr.startswith('_sa_instance_state'):  # Evita copiar el estado interno de SQLAlchemy
        ...
        self.assertTrue(verified_user.is_verified)  # Verify the user object is marked as verified
    ...
    async def test_verify_email_token_invalid_or_expired(self, mock_jwt_decode, mock_repository_auth,
        ...
        # Scenario 1: JWT decoding fails (e.g., malformed, wrong signature)
        ...
        mock_repository_auth.get_verification_token.assert_not_called()  # Use assert_not_called
        mock_repository_users.get_user_by_email.assert_not_called()  # Use assert_not_called
        mock_jwt_decode.reset_mock(side_effect=True)  # Reset for next scenario

        # Scenario 2: Token not found in DB
        ...
        mock_repository_auth.get_verification_token.reset_mock(return_value=None)  # Reset for next scenario
    ...
    async def test_verify_email_token_user_not_found(self, mock_jwt_decode, mock_repository_auth,
        ...
        mock_repository_auth.get_verification_token.return_value = mock_token_db
        mock_repository_users.get_user_by_email.return_value = None  # Simulate user not found
        ...
        mock_repository_users.update_user_is_verified.assert_not_called()  # No debería llamarse si el usuario no existe
    ...
    async def test_verify_email_token_already_verified(self, mock_jwt_decode, mock_repository_auth,
                                                       mock_repository_users):
        email_already_verified = self.mock_user.email  # Use the already verified mock_user
        mock_jwt_decode.return_value = {"sub": email_already_verified, "type": "email_verification"}

        # Simulate a token in DB for an already verified user
        mock_token_db = MagicMock(
            id=1,
            ...
            token_type="email_verification",
            expires_at=datetime.now(timezone.utc) + timedelta(hours=1),
            user=self.mock_user  # Associate the already verified user
        )

        mock_repository_auth.get_verification_token.return_value = mock_token_db
        mock_repository_users.get_user_by_email.return_value = self.mock_user  # User is already_verified=True
        ...
        mock_repository_users.update_user_is_verified.assert_not_called()  # <-- CAMBIO A assert_not_called()
        mock_repository_auth.delete_verification_token.assert_not_called()
    ...
    async def test_create_password_reset_token_and_save_user_not_found(self, mock_repository_users):
        mock_repository_users.get_user_by_email.return_value = None
        # This test now expects an HTTPException as the service function should raise it.
        with self.assertRaisesRegex(HTTPException, "User not found."):
    ...
    async def test_reset_password_success(self, mock_jwt_decode, mock_repository_users, mock_repository_auth,
        ...
        mock_reset_token_db = MagicMock(
            ...
            user=self.mock_user  # Ensure the token has the user associated if needed
        )
        mock_repository_auth.get_verification_token.return_value = mock_reset_token_db
        mock_repository_users.get_user_by_email.return_value = self.mock_user  # auth.py should fetch user by email from token
        mock_get_password_hash.return_value = "new_hashed_password"
        mock_repository_users.update_user_password.return_value = self.mock_user
        mock_repository_auth.delete_verification_token = AsyncMock()
        ...
        mock_repository_users.update_user_password.assert_awaited_once_with(self.mock_user.id, "new_hashed_password",
                                                                            self.mock_db)
    ...
    async def test_reset_password_user_not_found(self, mock_jwt_decode, mock_repository_auth, mock_repository_users):
        ...
        with self.assertRaisesRegex(HTTPException, "Invalid or expired reset token"):  # <-- CAMBIO AQUÍ
        ...
        mock_repository_auth.get_verification_token.assert_awaited_once()
        mock_repository_users.get_user_by_email.assert_awaited_once_with(self.mock_db, email_from_payload)
        mock_repository_users.update_user_password.assert_not_called()  # <-- CAMBIO A assert_not_called()
        mock_repository_auth.delete_verification_token.assert_not_called()  # <-- CAMBIO A assert_not_called()
